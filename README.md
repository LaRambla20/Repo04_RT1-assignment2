...
The folder in which this README is contains a ROS package, named `second_assignment_controller`, and a folder with the package documentation generated by the software doxygen. Inside the package there are three additional folders, a text file and a file with extension .xml:
* `include`: folders that contain everything that the used simulator needs to be run
